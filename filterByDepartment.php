...
<body>
<?php if (isset($_SESSION['filterByDepartment'])){
	if (empty($filterByDepartment))	$filterByDepartment = $_SESSION["filterByDepartment"];?>
	    ...
	    <td  class="libList">
		    <form method='post'>
				<?php 
					...
					$offset = $page * 5;
					$q="select libraries.Id,libraries.name from libraries,uoadepartments where libraries.department = uoadepartments.Id and uoadepartments.Name = '".$filterByDepartment."' limit $offset, 5;";
					...
					echo '<div><h4 class="libTitle"><b>Αποτελέσματα αναζήτησης ("'.$filterByDepartment.'")</b></h4>';
				    if($num > 0) {
						...
						echo '<div class="tableDiv">Δε βρέθηκαν αποτελέσματα</div>';
					}
				?>
			</form>
	    </td>
